...
import { setName, clearForm, addActor, setError, setSuccess, editActor } from '../../../../controller/SliceReducer/addActor';

const AddActor = () => {
    const dispatch = useDispatch();
    const form = useSelector((state) => state.actor);
    const { name, loading, error, success, isEdit, id } = form;
        ...
        <div>
            <div className="dark:bg-gray-900 bg-gray-100 flex justify-center items-center mt-10 mb-10">
                <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-md shadow-lg">
                    {isEdit ? (
                        <h2 className="text-3xl font-semibold text-center mb-6 dark:text-white">Cập nhập diễn viên</h2>
                    ) : (
                        <h2 className="text-3xl font-semibold text-center mb-6 dark:text-white">Thêm diễn viễn</h2>
                    )}
