...
import { setName, clearForm, addCategory, editCategory, setSuccess, setError } from '../../../controller/SliceReducer/addCategory';

const AddCategory = () => {
    const dispatch = useDispatch();
    const form = useSelector((state) => state.category);
    const { name, loading, error, success, id, isEdit } = form;
        ...
        <div>
            <div className="dark:bg-gray-900 bg-gray-100 flex justify-center items-center mt-10 mb-10">
                <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-md shadow-lg">
                    {isEdit ? (
                        <h2 className="text-3xl font-semibold text-center mb-6 dark:text-white">Sửa thể loại</h2>
                    ) : (
                        <h2 className="text-3xl font-semibold text-center mb-6 dark:text-white">Thêm thể loại</h2>
                    ...
                    <form className="" onSubmit={isEdit ? handleUpdate : handleSubmit}>
                        ...
                        {success && <div className="mt-4 text-green-500">Thành công!</div>}
                    </form>
                </div>
            </div>
        </div>
