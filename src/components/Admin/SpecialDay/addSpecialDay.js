...
import { addSpecialDay, setName, clearForm, setSuccess, setError, editDate } from "../../../controller/SliceReducer/specialDay";
import MyDatePicker from "./selectDay";

const AddSpecialDay = () => {
    const dispatch = useDispatch();
    const form = useSelector((state) => state.special);
    const { name, dateStart, dateEnd, loading, error, success, id, isEdit } = form;
                ...
                <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-md shadow-lg">
                    ...
                    <form className="" onSubmit={isEdit ? handleUpdate : handleSubmit}>
                        <div className="relative border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-md px-8  py-8 space-y-5">
                            <div>
                                <label htmlFor="name">Ngày đặc biệt</label>
