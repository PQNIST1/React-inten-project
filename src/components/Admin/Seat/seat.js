...
import { setEarly, setNormal, setSpecial, setWeekend, clearForm, deleteSeatType, setSuccess, setError, setName, setCode, setDateEnd, setDateStart, setEdit, setId, setPrice, setType_id, setDay } from "../../../controller/SliceReducer/seat";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faPenToSquare } from "@fortawesome/free-regular-svg-icons";
import { splitDateTime } from "../../../controller/SliceReducer/img";

const getPriceBySeatTypeId = (data, seatTypeId) => {
...
const Seat = ({ data, prices, pp }) => {
    ...
    const [normal, setNormal1] = useState('');
    const [weekend, setWeekend1] = useState('');
    const [special, setSpecial1] = useState('');
    const [early, setEarly1] = useState('');
    const dispatch = useDispatch();
    const form = useSelector((state) => state.seat);
    const { isEdit, type_id } = form;
    useEffect(() => {
        setPrices(getPriceBySeatTypeId(prices, data.id).price);
        setStart(getPriceBySeatTypeId(prices, data.id).start);
        setEnd(getPriceBySeatTypeId(prices, data.id).end);
        setNormal1(getPriceBySeatTypeId(prices, data.id).normal);
        setWeekend1(getPriceBySeatTypeId(prices, data.id).weekend);
        setSpecial1(getPriceBySeatTypeId(prices, data.id).special);
        setEarly1(getPriceBySeatTypeId(prices, data.id).early);
        setId1(getPriceBySeatTypeId(prices, data.id).id);
    }, [prices, data]);

    const handleDelete = (id) => {
        dispatch(deleteSeatType({ id, id1 }));
        setTimeout(() => {
            dispatch(setSuccess());
            dispatch(setError());
        }, 3000);
    ...
    const handleUpdate = (idd) => {
        if (isEdit && idd === type_id) {
            dispatch(setEdit(false));
            dispatch(clearForm());
        } else {
            dispatch(setId(id1));
            dispatch(setType_id(idd));
    ...
    return (
        <tr className=" transition-all duration-500  text-gray-400 ">
            ...
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{price}</td>
            ...
            <td className=" p-5 ">
                <div className="flex items-center gap-1">
                    <button onClick={() => handleUpdate(data.id)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                        <FontAwesomeIcon icon={faPenToSquare} className={`${isEdit && type_id === data.id ? 'text-orange-500' : 'text-indigo-500'}`} />
                    </button>
                    <button onClick={() => handleDelete(data.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
