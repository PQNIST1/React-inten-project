...
import { setSuccess, setError, setEdit, clearForm, setId, deleteShowTime, setMovie, setRoom } from "../../../controller/SliceReducer/addShowTime";
...
const ShowTime = ({ data, pp }) => {
    ...
    const transformedMovie = {
        value: data.movie.id,
        label: data.movie.name
    };

    // Chuyển đổi đối tượng room
    ...
    const handleUpdate = (idd) => {
        if (isEdit && idd === id) {
            ...
            dispatch(setMovie(transformedMovie));
            dispatch(setRoom(transformedRoom));
            dispatch(setDateStart(data.startTime));
            dispatch(setDateEnd(data.endTime));
        }
    ...
    return (
        <tr className=" transition-all duration-500  text-gray-400 ">
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{data.movie.name}</td>
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{data.room.name}</td>
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{formatDate(data.startTime)}</td>
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{formatDate(data.endTime)}</td>
            <td className="p-5 whitespace-nowrap  leading-6 font-medium text-tn">
                {pp.createdBy && (<div>
            ...
            <td className=" p-5 ">
                <div className="flex items-center gap-1">
                    <button onClick={() => handleUpdate(data.id)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                        <FontAwesomeIcon icon={faPenToSquare} className={`${isEdit && id === data.id ? 'text-orange-500' : 'text-indigo-500'}`}
                        />
                    </button>
