...
const Tail = ({ data, index }) => {
    ...
    const transformedRoom = {
        value: data.object.room.id,
        label: data.object.room.name
    };
    ...
    return (
        <tr className=" transition-all duration-500  text-gray-400 ">
            ...
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{formatDate(data.object.startTime)}</td>
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{formatTime(data.object.startTime)}</td>
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{formatTime(data.object.endTime)}</td>
            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">{data.object.room.name}</td>

            <td className="p-5 whitespace-nowrap  leading-6 font-medium text-tn">
                {data.createdBy && (<div>
                    <p>{data.createdBy.name}</p>
                    <p> {splitDateTime(data.createdAt)}</p>
                </div>)}</td>
            <td className="p-5 whitespace-nowrap leading-6 font-medium text-tn">{data.updatedBy && (
                <div>
                    <p>{data.updatedBy.name}</p>
                    <p> {splitDateTime(data.updatedAt)}</p>
                </div>
            )}</td>
            <td className=" p-5 ">
                <div className="flex items-center gap-1">
                    <button  className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                        <FontAwesomeIcon icon={faPenToSquare} onClick={() => handleUpdate(data.object.id)} className={`${isEdit && id === data.object.id ? 'text-orange-500' : 'text-indigo-500'}`}
                        />
                    </button>
                    <button onClick={() => handleDelete(data.object.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
