...
import { getMonthReport } from "../../../../controller/SliceReducer/report";
...
const MonthList = () => {
    ...
    const months = useSelector((state) => state.report.month);
    ...
    const month = String(today.getMonth() + 1).padStart(2, '0'); // Lấy tháng hiện tại và đảm bảo có 2 chữ số
    const formattedDate = `${year}-${month}`;
    const dateStart = useSelector((state) => state.special.dateStart);
    const dateEnd = useSelector((state) => state.special.dateEnd);
    useEffect(() => {
        if (dateStart && dateEnd) {
            dispatch(getMonthReport({ fromDate: dateStart.split('T')[0], toDate: dateEnd.split('T')[0] }));
        } else {
            dispatch(getMonthReport({ fromDate: formattedDate, toDate: formattedDate }));
        }
    }, [dispatch, formattedDate, dateStart, dateEnd]);
    useEffect(() => {
                        ...
                        <table className=" min-w-full ">
                            <thead>
                                <tr className=" text-gray-400 bg-gray-50 bg-opacity-10">
                                    <th scope="col" className="  p-5 text-left text-sm leading-6 font-semibold capitalize"> Tháng </th>
