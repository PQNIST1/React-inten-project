...
import { getDailyReport} from "../../../../controller/SliceReducer/report";
...
const DailyList = () => {
    ...
    const dailys = useSelector((state) => state.report.daily);
    ...
    useEffect(() => {
        if (dateStart && dateEnd) {
            dispatch(getDailyReport({ fromDate: dateStart.split('T')[0], toDate: dateEnd.split('T')[0] }));
        } else {
            dispatch(getDailyReport({ fromDate: today, toDate: today }));
        }
